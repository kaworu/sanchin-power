...
# SimpleCov setup, must be done before loading our app code.
if ENV['COVERAGE']
  require 'simplecov'
  SimpleCov.start do
    add_filter '/public/'
...
module RSpecHelper
  ...
  def patch_json(uri, body)
    header 'content-type', 'application/json'
    patch uri, body.to_json
  end

  def put_json(uri, body)
    header 'content-type', 'application/json'
    put uri, body.to_json
  end
...
=end
  ...
  config.around(:each, transaction: true) do |example|
